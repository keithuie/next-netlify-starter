// Script to geocode all site locations once
const fs = require('fs');
...
async function geocodePostalCode(postalCode) {
  ...
  if (data && data.length > 0) {
    console.log(`✓ Geocoded ${postalCode}`);
    return {
  ...
  console.error(`✗ Failed to geocode: ${postalCode}`);
  return null;
}

async function geocodeAll() {
  const lines = csvData.trim().split('\n');
  const results = ['Site Name,What3Words Location,Postal Code,Region,Sub-region,Asset Count,Latitude,Longitude'];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    console.log(`[${i}/${lines.length - 1}] Geocoding ${postalCode}...`);

    const coords = await geocodePostalCode(postalCode);

    if (coords) {
      results.push(`${line},${coords.lat},${coords.lng}`);
    } else {
      results.push(`${line},ERROR,ERROR`);
    }
  ...
  // Save to file
  fs.writeFileSync('geocoded-sites.csv', results.join('\n'));
  console.log('\n✓ Saved to geocoded-sites.csv');
}
