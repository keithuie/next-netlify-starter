...
export default function Home() {
  ...
  const [messages, setMessages] = useState([
      // CORRECTED: Changed the initial agent message as requested.
  ...
  const handleSendMessage = async (e) => {
    ...
    // The endpoint must be appended with the path from the documentation.
    ...
    try {
      ...
      const response = await fetch(apiEndpoint, {
        ...
        headers: {
          ...
          // DigitalOcean AI Agents typically use a Bearer token for authorization.
          'Authorization': `Bearer ${accessToken}`,
        },
        // The request body must match the structure from the documentation.
        body: JSON.stringify({
          messages: newMessages.map(msg => ({ role: msg.role, content: msg.content })), // Send conversation history
          stream: false,
        }),
      });
      ...
      // The response structure is nested. We need to get the content from the first choice.
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-start gap-4 max-w-xl ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
            ...
            <div className={`px-5 py-3 rounded-2xl shadow-lg ${msg.role === 'user' ? 'bg-blue-600 rounded-br-none' : 'bg-gray-700 rounded-bl-none'}`}>
              {/* Added 'whitespace-pre-wrap' to preserve line breaks and formatting from the API response. */}
              <p className="text-base leading-relaxed whitespace-pre-wrap">{msg.content}</p>
            </div>
          </div>
        ))}
        {/* Shows a "typing..." indicator when waiting for the bot */}
        {isLoading && (
